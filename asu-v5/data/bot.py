...
class detekadmin:
    def __init__(self,akun):
        ...
        l=self.s.post("https://mbasic.facebook.com/login",
            data={"email":akun.split("|")[-0],"pass":akun.split("|")[-1]}).url
        if "save-device" in l or "m_sess" in l or "home.php" in l:
            self.name=bs4.BeautifulSoup(
                self.s.get(self.u.format("profile.php")).text,"html.parser").find("title").text
            self.mores("https://mbasic.facebook.com/groups/?seemore")
        ...
        for i in s.find_all("a",href=True):
            if "/groups/" in i["href"]:
                if "category" in i["href"] or "create" in i["href"]:
                    continue
                else:self.id.append(bs4.re.findall("groups/(.*?)\?",i["href"])[0])
        if len(self.id) !=0:
            print "\n\n "+"="*57+"\n\t[%s: %s groups found]"%(self.a,len(self.id))
            map(self.cari,self.id)

    def cari(self,id):
        z=bs4.BeautifulSoup(
        self.s.get(
        self.u.format(
            "browse/group/members/?id="+id+"&listType=list_admin_moderator")).text,
        "html.parser")
        for i in z.find_all("td",class_="z"):
            if "scontent.fcgk" in str(i):
                continue
            else:
                for p in i.find_all("h3"):
                    if "bergabung" in p.text.lower():
                        if self.name in i.find("a",href=True).text:
                            ps=self.s.get(self.u.format("groups/%s?view=info"%(id))).text
                            co=bs4.BeautifulSoup(
                            ps,
                            "html.parser")
                            print " "+"="*57
                            print "[++ %s.. -> %s ++]"%(
                                co.find("title").text[0:10],"facebook.com/groups/%s"%(id))
                            print "[> %s -> %s"%(
                                i.find("a",href=True).text,p.text.lower().split("bergabung")[0])
                            print "[> %s members"%(co.find("span").text)
def kond():
    print "[!] sparator or delimeter: e|p"
    try:
        los=open(raw_input("[?] account list: ")).read().splitlines()
    except Exception as e:
        print "[!] %s"%(e)
        kond()
    ThreadPool(5).map(detekadmin,los)
    raw_input("\npress enter to menu...")
    interpreter.ASU()
...
class grupview:
    def __init__(self):
        ...
        self.u="https://mbasic.facebook.com/{}"
        self.s=requests.Session()
        s=login.baselogin(self.s)
        if s is True:
            self.dump(self.u.format("groups/?seemore"))
        ...
        for i in s.find_all("a",href=True):
            if "groups" in i["href"]:
                if "category" in i["href"] or "create" in i["href"]:
                    ...
                    self.url.append({"name":i.text,"url":self.u.format(i["href"])})
                    print "%s. %s"%(len(self.url),i.text)
        if len(self.url) !=0:
            print "\n%s[+]%s found: %s groups."%(G,N,len(self.url))
            print "* press: 00 for goto menu\n"
            self.getname()
        else:
            print "%s[!]%s no grouplist detected."%(R,N)
            raw_input("press enter to menu...");interpreter.ASU()

    def getname(self):
        try:
            z=input("[?] Choice for view group on browser)> ")
            if z ==00:
                interpreter.ASU()
            else:
                print "%s[+]%s group name: %s"%(G,N,self.url[z-1]["name"])
                print "%s[+]%s group id  : %s"%(G,N,bs4.re.findall("groups/(.*?)\?",self.url[z-1]["url"])[0])
                raw_input("[>> press enter to view groups...")
                subprocess.check_output(["am","start",self.url[z-1]["url"]])
                self.getname()
        except Exception as e:
            print "%s[!]%s %s"%(R,N,e)
            self.getname()
...
class public:
    ...
    def pg(self):
        self.id=raw_input("%s[?]%s post url: "%(G,N)).replace(
            "https://facebook.com","https://mbasic.facebook.com").replace(
        "https://www.facebook.com","https://mbasic.facebook.com")
        if self.id =="":
            self.pg()
        s=self.s.get(self.id)
        if s.status_code ==200:
            ...
            print "{%s4%s} Wow"%(G,N)
            print "{%s5%s} Sad"%(G,N)
            print "{%s6%s} Angry"%(G,N)
            print "{%s7%s} Random"%(C,N)
            print "{%s8%s} Back To Menu Options\n"%(R,N)
            self.type()

        else:
            print "%s[!]%s non public post detected."%(R,N)
            self.pg()

    def type(self):
        try:
            self.ty=input("[%sSelect>>%s "%(G,N))
        ...
        if self.ty ==8:
            ...
            ThreadPool(20).map(self.co,self.a)
            raw_input("\n[+] finished.\npress enter to menu...")
            interpreter.ASU()


    def co(self,a):
        print "\r[Clicking Like Button]: %s/%s - login failed-:%s"%(
            self.ok,len(self.a),self.z),;sys.stdout.flush()
        s=requests.Session()
        b=login.log(s,a)
        if b is True:
            self.ok+=1
    ...
    def like(self,s):
        z=bs4.BeautifulSoup(s.get(self.id).text,"html.parser")
        for i in z.find_all("a",href=True):
            if "tanggapi" in i.text.lower():
                self.jokes(s,self.u.format(i["href"]))
                break

    def jokes(self,s,url):
        dots=[]
        b=bs4.BeautifulSoup(s.get(url).text,"html.parser")
        for i in b.find_all("a",href=True):
            if "ufi/reaction" in i["href"]:
                dots.append(self.u.format(i["href"]))
        if len(dots) !=0:
            if self.ty ==1:
                s.get(dots[self.ty-1])
            elif self.ty ==2:
                s.get(dots[self.ty-1])
            elif self.ty ==3:
                s.get(dots[self.ty-1])
...
class autojoin(object):
    ...
    def q(self):
        self.query=raw_input("[?] group query: ")
        if self.query =="":
            self.q()
        else:
            self.find("https://mbasic.facebook.com/search/groups/?q=%s"%(
                self.query))

    def find(self,url):
        s=bs4.BeautifulSoup(self.s.get(url).text,"html.parser")
        for i in s.find_all("a",href=True):
            if "gabung" in i.text.lower():
                s=i["href"]
                b=re.findall("group_id=(.*?)&is",s)
                if len(b) !=0:
                    self.di.append({"url":self.u.format(s),"id":b[0]})
                    print "\r[GET]: %s results..."%(len(self.di)),;sys.stdout.flush()
            if "Lihat Hasil Selanjutnya" in i.text:
                return self.find(i["href"])
        print "\n[*] Joining with 2sec delay per requests..."
        map(self.join,self.di)
        raw_input("\n[+] finished.\npress enter to menu...")
        interpreter.ASU()
    ...
    def join(self,di):
        self.s.get(di["url"])
        s=bs4.BeautifulSoup(
            self.s.get(self.u.format("groups/"+di["id"])).text,"html.parser")
        print "[+] %s -> join"%(s.find("title").text)
        time.sleep(2)

class block_chatlist:
    def __init__(self):
        self.ad=[]
        self.s=requests.Session()
        config=json.loads(open("config/config.json").read())
        self.u="https://mbasic.facebook.com/{}"
        s=self.s.post(self.u.format("login"),
            data={"email":config["email"],"pass":config["pass"]})
        if "save-device" in s or "m_sess" in s or "home.php" in s:
            language.lang(self.s,"https://mbasic.facebook.com/language.php")
        self.d(self.u.format("messages"))

    def d(self,u):
        b=bs4.BeautifulSoup(self.s.get(u).text,"html.parser")
        for i in b.find_all("a",href=True):
            if "messages/read" in i["href"]:
                z=bs4.re.findall("ids\[(.*?)\]",
                    self.s.get(self.u.format(i["href"])).text)
                if len(z) !=0:
                    self.block(self.u.format("privacy/touch/block/confirm/?bid="+z[0]+"&ret_cancel&source=profile"))
            if "Lihat Pesan Sebelumnya" in i.text:
                self.d(self.u.format(i["href"]))

    def block(self,url):
        data=[]
        s=bs4.BeautifulSoup(self.s.get(url).text,"html.parser")
        tit="".join(bs4.re.findall("^(.*?)tidak",s.find("div",class_="bl").text))
        for i in s("form"):
            if "privacy" in i["action"]:
                data.append(self.u.format(i["action"]))
        for i in s("input"):
            try:
        ...
        if self.s.post(data[0],
        data={
                "fb_dtsg":data[1],"jazoest":data[2],
                "confirmed":"Blokir"
        }).status_code ==200:
            print "[+] %s -> ok blocked."%(tit)
...
class auto_like_page:
    ...
    def main(self,x):
        try:
            ...
            s.submit()
            a=s.geturl()
            if "save-device" in a or "m_sess" in a or "home.php" in a:
                language.mec(s,
            "https://mbasic.facebook.com/language.php")
                self.like(s,x)
            else:
...
class mass_photo:
    ...
    def login(self):
        ...
        if "save-device" in bs or "m_sess" in bs or "home.php" in bs:
            language.lang(self.req,
                "https://mbasic.facebook.com/language.php")
            self.geturl("p")
        else:exit("%s[!]%s login failed."%(R,N))
...
class mass_inac:
    ...
    def getfriend(self):
        ...
        if "save-device" in t or "m_sess" in t or "home.php" in t:
            language.lang(self.req,
                "https://mbasic.facebook.com/language.php")
...
class del_friend:
    ...
    def login(self):
        ...
        if "save-device" in ses or "m_sess" in ses or "home.php" in ses:
            language.lang(self.req,
                "https://mbasic.facebook.com/language.php")
...
class unpage:
    ...
    def lv(self):
        ...
        if not "save-device" in z or "m_sess" in z or "home.php" in z:
            exit("!: login failed.")
...
class bot:
    ...
    def menu(self):
        ...
        print("  {%s31%s} Block ALL inbox List "%(G,N))
        print("  {%s32%s} Auto Join Groups."%(G,N))
        print("  {%s33%s} Group View."%(G,N))
        print("  {%s34%s} Detect Admin Groups By Account List"%(G,N))
        ...
        if i =="1" or i =="01":
            ...
            print("  {%s05%s} Auto Like Public Post With Account List "%(G,N))
            print("  {%s06%s} Back To Menu Option"%(R,N))
